...
// 관측기 설계
void observeMotor(const uint8 chn, const float32 v_in, const float32 theta)
{
    /*
     * chn : 인덱스
     * v_in : 입력
    ...
    // 각속도, 전류 적분 -> 각속도 획득
    g_int_w_h[chn] += ((Kt / J * g_i_h[chn]) + (-B / J * g_w_h[chn]) + (theta_tilde * L2)) * T_S;
    g_w_h[chn] = g_int_w_h[chn];

    // 각속도, 전류 적분 -> 전류 획득
    g_int_i_h[chn] += ((-R / L * g_i_h[chn]) + (-Kb / L * g_w_h[chn]) + (v_in / L) + (theta_tilde * L3)) * T_S;
    g_i_h[chn] = g_int_i_h[chn];
}

// Θ : 위상
float32 getThetaHat(const uint8 chn)
{
    return g_theta_h[chn];
}

// w : 각속도
float32 getWHat(const uint8 chn)
{
    return g_w_h[chn];
}

// I : 전류
float32 getIHat(const uint8 chn)
{
    return g_i_h[chn];
}
